...
class MultiModal(nn.Module):
    ...
    def _make_modality(self, residuals=False):
        if residuals:
            return ResNet(BasicResNetBlock, [1,1,1,1]) # TODO)
        else:
            return ResNet(BasicBlock, [1,1,1,1])


def multimodal18(pretrained=False, **kwargs):
    """Constructs a MultiModal-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MultiModal(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
